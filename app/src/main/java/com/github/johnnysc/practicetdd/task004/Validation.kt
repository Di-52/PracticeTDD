package com.github.johnnysc.practicetdd.task004
...
interface Validation {
    ...
    interface Matcher {
        ...
        class Length(private val minCont: Int) : Abstract(minCont,
            Result.MinLengthInsufficient(minCont),
            { true }) {
            override fun match(str: String): Result = super.match(str)
        }

        class NumbersCount(private val minCont: Int) : Abstract(minCont,
            Result.NumbersCountInsufficient(minCont),
            { it.isDigit() }) {
            override fun match(str: String): Result = super.match(str)
        }

        class UpperCaseLettersCount(private val minCont: Int) : Abstract(minCont,
            Result.UpperCaseLettersCountInsufficient(minCont),
            { it.isUpperCase() }) {
            override fun match(str: String): Result = super.match(str)
        }

        class LowerCaseLettersCount(private val minCont: Int) : Abstract(minCont,
            Result.LowerCaseLettersCountInsufficient(minCont),
            { it.isLowerCase() }) {
            override fun match(str: String): Result = super.match(str)
        }

        class SpecialSignsCount(private val minCont: Int) : Abstract(minCont,
